...
| 캐시·크레딧 정보 | 켜기 / 끄기 |
| 페이스 마커 | 켜기 / 끄기 |
| 갱신 주기 | 수동, 1분 ~ 1시간 (기본 5분) |
| 시계 | 숨김, 좌측 하단 / 가운데 / 우측 하단 |
...
캐시해 두는데, 실시간 조회가 불가능할 때는 위젯도 이 값으로 대체합니다. 다만 캐시만
믿을 수는 없습니다. Claude Code는 `/usage`를 실행할 때만 이 값을 갱신하기 때문에
며칠씩 묵은 값이 남아 있을 수 있습니다.
...
주황을 앞세운 검증된 색상을 대신 씁니다. 상태 색상은 두 테마에서 고정이며 항상
상태만 의미하고 항목 구분에는 쓰이지 않습니다.
