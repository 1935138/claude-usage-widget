...
- **여러 계정.** 한도는 계정 단위입니다. 설치본들이 서로 다른 계정으로 로그인되어
  있으면 드롭다운으로 골라 볼 수 있고, 설정 패널에서 계정을 더 추가할 수도
  있습니다. 두 번째 설치본을 만들 필요가 없습니다.
- **로그인 안 되어 있으면 버튼 하나로.** 자격 증명이 없으면 `claude auth login`을
  실행하는 버튼을 대신 보여줍니다.
...
원하는 자리로 옮겨두면 다음에 켤 때도 그 자리, 그 모니터에 뜹니다. 그 모니터가
없어졌으면 화면 밖이 아니라 연결된 모니터 위에 뜹니다.
...
설정 패널의 계정 추가는 위젯 전용 설정 디렉터리를 하나 만들고 그곳을 향해
`claude auth login`을 실행합니다. 평소 쓰시는 설치본을 건드리지 않고 다른 계정의
잔량을 지켜볼 수 있습니다. 자격 증명은 Claude Code가 직접 그 폴더에 씁니다. 위젯이
토큰을 쓰는 일은 여전히 없지만, 그 위치를 고른 것은 위젯입니다. 제거를 누르면
계정을 잊고 그 폴더도 지웁니다.

이 일은 `CLAUDE_CONFIG_DIR`로는 할 수 없습니다. CLI가 그 값 전체를 경로 하나로
읽기 때문에, 두 폴더를 적어 넣으면 모든 `claude` 실행이 둘을 이어 붙인 이름의
폴더를 찾게 됩니다. 그래서 목록은 위젯 자신의 `settings.json`에 두고, 그 환경
변수는 로그인 프로세스 하나에만 건넵니다.
